...
export default function UploadPage() {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await api.post('/posts', { imageUrl: publicUrl, price: Number(price) });
                ...
                <form onSubmit={handleSubmit} className="space-y-8">
                    <div>
                        ...
                        <div className="relative aspect-square border-2 border-dashed border-gray-300 rounded-xl flex items-center justify-center bg-gray-50 hover:bg-gray-100 transition-colors">
                            {preview ? (
                                <Image src={preview} alt="Upload preview" className="w-full h-full object-cover rounded-xl" />
                            ) : (
                    ...
                    <div>
                        ...
                        <input type="number" id="price" value={price} onChange={(e) => setPrice(e.target.value)} placeholder="0.1" step="0.01" min="0" required className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-purple-500" />
                    </div>
                    <button type="submit" disabled={!file || !price || isUploading} className="w-full bg-purple-600 text-white py-4 rounded-lg font-medium hover:bg-purple-700 transition-colors disabled:opacity-50 flex items-center justify-center gap-2">
